		...
		$.ajax({
			...
			success: (res) => {
				...
				if(res.code != 0){
					...
					reElem.find(`dd.${FORM_NONE}`).removeClass(FORM_EMPTY);
					//获得已选择的values
					let spans = reElem.find(`.${LABEL} span`).toArray().map((item) => {
						return {
...
//			<li class="xm-select-this xm-select-active"><span>123</span></li>
		...
		let values = [];
		reElem.find('dl').html(this.renderSelect(pcInput.attr('placeholder') || pcInput.attr('back'), dataArr.map((item) => {
...
//		arr.push(`<dd lay-value="" class="layui-select-tips">各种复杂的地方操作</dd>`);
	...
	Common.prototype.removeAll = function(id){
		...
		data[id].values.concat([]).forEach((item, index) => {
			this.handlerLabel(id, dl.find(`dd[lay-value="${item.val}"]`), false, item, true);
		});
	}
